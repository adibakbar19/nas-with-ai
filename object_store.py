...
class ObjectStoreSettings:
    ...
    def from_env(cls, *, env: dict[str, str] | None = None) -> "ObjectStoreSettings":
        ...
        auto_create_bucket = _env_bool(
            "OBJECT_STORE_AUTO_CREATE_BUCKET",
        ...
        return cls(
            bucket=_env_first("OBJECT_STORE_BUCKET", "S3_BUCKET", "AWS_S3_BUCKET", "MINIO_BUCKET", env=env)
            or "nas-uploads",
            region=_env_first("AWS_REGION", "OBJECT_STORE_REGION", "S3_REGION", env=env) or "us-east-1",
            endpoint=endpoint,
            public_endpoint=public_endpoint,
            access_key_id=_env_first("AWS_ACCESS_KEY_ID", "OBJECT_STORE_ACCESS_KEY", "MINIO_ACCESS_KEY", env=env),
...
def ensure_bucket_exists(client, settings: ObjectStoreSettings) -> None:
    try:
        client.head_bucket(Bucket=settings.bucket)
        return
    except Exception as exc:
        if not settings.auto_create_bucket or not is_missing_object_error(exc):
            raise

    kwargs: dict[str, Any] = {"Bucket": settings.bucket}
    if settings.region and settings.region != "us-east-1":
