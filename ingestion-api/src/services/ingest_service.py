"""Ingest service — file upload, multipart, job lifecycle.

All dependencies injected via constructor. No module-level singletons.
No worker-only operations (run_job is excluded).
"""
...
from src.storage.object_store import ObjectStoreSettings, build_s3_client, is_missing_object_error

logger = logging.getLogger(__name__)


class IngestService:
    """Handles file upload, multipart upload, and job lifecycle for ingestion."""
    ...
    def __init__(
        ...
        *,
        ...
        multipart_repo: MultipartUploadRepository,
        idempotency_repo: ApiIdempotencyRepository,
        object_store: ObjectStoreSettings,
        output_dir: Path,
    ) -> None:
        self._job_state = job_state
        self._multipart_repo = multipart_repo
    ...
    def _build_retry_corrections_key(
        cls, *, agency_id: str, parent_job_id: str, retry_job_id: str, file_name: str
    ) -> str:
    ...
    def _get_s3_client(self, *, public_endpoint: bool = False):
        return build_s3_client(self._store, public=public_endpoint)

    def _object_store_public_endpoint(self) -> str:
        endpoint = os.getenv("OBJECT_STORE_PUBLIC_ENDPOINT", "").strip() or self._store.public_endpoint
        if not endpoint:
            return ""
        secure = os.getenv("OBJECT_STORE_PUBLIC_SECURE", str(self._store.secure)).lower() in {
            "1", "true", "yes", "y",
        }
    ...
    def _ensure_bucket_cors(self) -> None:
        ...
        if not manage_cors:
            manage_cors_enabled = bool(self._store.endpoint)
        else:
        ...
        cors_raw = os.getenv("CORS_ALLOW_ORIGINS", "http://localhost:3000,http://localhost:5173")
    ...
    def _claim_idempotent_request(
        ...
        *,
        ...
        if not normalized_key:
            return None, None
        created, record = self._idempotency_repo.claim_request(
            agency_id=agency_id,
            operation=operation,
    ...
    def _complete_idempotent_request(
        ...
        *,
        ...
        if not normalized_key:
            return
        self._idempotency_repo.complete_request(
            agency_id=agency_id,
            operation=operation,
            idempotency_key=normalized_key,
    ...
    def _release_idempotent_request(self, *, agency_id: str, operation: str, idempotency_key: str | None) -> None:
        ...
        if not normalized_key:
            return
        self._idempotency_repo.delete_request(
            agency_id=agency_id,
            operation=operation,
            idempotency_key=normalized_key,
        )
    ...
    def _queue_or_run(self, job_id: str) -> None:
        try:
            self._job_state.queue_ingest_job(job_id)
        except Exception as exc:
            self._job_state.set_job(
                job_id,
                status="failed",
                ended_at=IngestJobState.now_iso(),
    ...
    def _register_uploaded_object(
        ...
        *,
        ...
        source_type = self.infer_source_type(file_name)
        job_id = str(job_id or uuid.uuid4().hex)
        success_path = self._output_dir / job_id / "cleaned"
        warning_path = self._output_dir / job_id / "warnings"
        failed_path = self._output_dir / job_id / "failed"
        checkpoint_root = self._output_dir / job_id / "checkpoints"

        self._job_state.set_job(
            job_id,
            agency_id=agency_id,
            status="uploaded",
            created_at=IngestJobState.now_iso(),
            file_name=file_name,
            ...
            manifest_object_name=self._build_job_manifest_key(agency_id=agency_id, job_id=job_id),
            bucket=bucket,
        ...
        if auto_start:
            self._job_state.set_job(job_id, status="queued")
    ...
    def upload(
        ...
        *,
        ...
        try:
            ...
            logger.info("upload_complete job_id=%s bytes=%d method=direct", job_id, content_bytes)
    ...
    def retry_failed_rows_upload(
        ...
        *,
        ...
        load_to_db = True
        parent_job = self._job_state.get_job(parent_job_id, agency_id=agency_id)
        if not parent_job:
        ...
        success_path = self._output_dir / parent_job_id / "retry_failed_rows" / retry_job_id / "cleaned"
        warning_path = self._output_dir / parent_job_id / "retry_failed_rows" / retry_job_id / "warnings"
        failed_path = self._output_dir / parent_job_id / "retry_failed_rows" / retry_job_id / "failed"
        ...
        try:
            ...
            logger.info("upload_complete job_id=%s bytes=%d method=retry_corrections", retry_job_id, content_bytes)
            result = {
                "job_id": retry_job_id,
                "agency_id": agency_id,
                "parent_job_id": parent_job_id,
    ...
    def initiate_multipart_upload(
        ...
        *,
        ...
        try:
            self._multipart_repo.create_session(session=session)
            result = {
                "session_id": session_id,
                "agency_id": agency_id,
                "job_id": job_id,
    ...
    def get_multipart_upload_status(self, session_id: str, *, agency_id: str) -> dict[str, Any]:
        session = self._multipart_repo.get_session(session_id=session_id, agency_id=agency_id)
        if not session:
    ...
    def get_multipart_part_url(self, *, session_id: str, part_number: int, agency_id: str) -> dict[str, Any]:
        ...
        self._multipart_repo.update_session(session_id=session_id, status="uploading")
        return {
            "session_id": session_id,
    ...
    def complete_multipart_upload(
        ...
        *,
        ...
        logger.info("upload_complete job_id=%s bytes=%d method=multipart", result["job_id"], int(session.get("content_bytes") or 0))
        self._multipart_repo.update_session(
            session_id=session_id,
            status="completed",
    ...
    def abort_multipart_upload(self, *, session_id: str, agency_id: str) -> dict[str, Any]:
        ...
        self._multipart_repo.update_session(session_id=session_id, status="aborted")
    ...
    def start_job(self, job_id: str, *, agency_id: str, idempotency_key: str | None = None) -> dict[str, Any]:
        ...
        if replay is not None:
            return replay
        job = self._job_state.get_job(job_id, agency_id=agency_id)
        if not job:
            self._release_idempotent_request(
                agency_id=agency_id,
                operation="start_job",
    ...
    def list_jobs(self, *, agency_id: str, limit: int) -> dict[str, Any]:
        rows = self._job_state.list_jobs(agency_id=agency_id, limit=limit)
        return {"count": len(rows), "items": rows}

    def get_job(self, job_id: str, *, agency_id: str) -> dict[str, Any]:
        row = self._job_state.get_job(job_id, agency_id=agency_id)
        if not row:
            raise ServiceError(status_code=404, detail="job_id not found")
        return row
