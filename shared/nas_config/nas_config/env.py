...
Copied from nas_core/config/env.py — canonical source is now here.
"""
...
def validate_backend_env(*, env: dict[str, str] | None = None, raise_on_error: bool = False) -> list[str]:
    ...
    required = ["ES_URL", "ES_INDEX", "NAS_AUDIT_LOG", "VALKEY_URL"]
    missing = _missing(ctx, required)
    bucket = ctx.get("OBJECT_STORE_BUCKET") or ctx.get("S3_BUCKET") or ctx.get("AWS_S3_BUCKET")
    if bucket is None or str(bucket).strip() == "":
        missing.append("OBJECT_STORE_BUCKET|S3_BUCKET|AWS_S3_BUCKET")
