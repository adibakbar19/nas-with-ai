...
from domain.replacements import ADDRESS_ABBREVIATION_REPLACEMENTS
...
def normalize_address_text(value: str | None, *, uppercase: bool = True) -> str:
    ...
    for src, dst in ADDRESS_ABBREVIATION_REPLACEMENTS:
        for token in (f"{src}.", src):
            text = re.sub(rf"(?<![A-Z0-9]){re.escape(token)}(?![A-Z0-9])", dst, text)
