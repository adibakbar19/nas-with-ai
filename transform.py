...
def _assign_admin_from_boundaries(
    ...
    *,
    ...
    if state_name_col and state_code_col:
        ...
        df = state_join.withColumn("_state_from_boundary", col("_b_state_code").isNotNull())
        df = df.withColumn("state_code", coalesce(col("_b_state_code"), col("state_code")))
        df = df.withColumn("state_name", coalesce(col("_b_state_name"), col("state_name")))
    ...
    if district_name_col and district_code_col:
        ...
        df = district_join.withColumn("_district_from_boundary", col("_b_district_code").isNotNull())
        df = df.withColumn("state_code", coalesce(col("_b_district_state_code"), col("state_code")))
        df = df.withColumn("district_code", coalesce(col("_b_district_code"), col("district_code")))
        df = df.withColumn("district_name", coalesce(col("_b_district_name"), col("district_name")))
        df = df.drop(
            "_b_district_name",
            "_b_district_code",
    ...
    if mukim_name_col and mukim_code_col:
        ...
        df = mukim_join.withColumn("_mukim_from_boundary", col("_b_mukim_code").isNotNull())
        df = df.withColumn("state_code", coalesce(col("_b_mukim_state_code"), col("state_code")))
        df = df.withColumn("district_code", coalesce(col("_b_mukim_district_code"), col("district_code")))
        df = df.withColumn("district_name", coalesce(col("_b_mukim_district_name"), col("district_name")))
        df = df.withColumn("mukim_code", coalesce(col("_b_mukim_code"), col("mukim_code")))
        df = df.withColumn("mukim_name", coalesce(col("_b_mukim_name"), col("mukim_name")))
        df = df.withColumn("mukim_id", coalesce(col("mukim_id"), col("_b_mukim_id")))
        df = df.drop(
            "_b_mukim_name",
            "_b_mukim_code",
            "_b_mukim_id",
...
def clean_addresses(
    ...
    if (
        admin_boundary_enabled
        and os.path.exists(state_boundary_dir)
        and os.path.exists(district_boundary_dir)
        and os.path.exists(mukim_boundary_dir)
    ):
        try:
