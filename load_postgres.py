...
def _merge_packages(existing: str | None, package: str) -> str:
    if not existing:
        return package
    parts = [p.strip() for p in existing.split(",") if p.strip()]
    if not any(p.startswith("org.postgresql:postgresql:") for p in parts):
...
def main() -> None:
    ...
    try:
        ...
        jars_packages = _merge_packages(jars_packages, "org.postgresql:postgresql:42.7.3")
